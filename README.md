...
- Python 3.12.x（项目根目录 `.python-version` 固定为 `3.12`，脚本会拒绝 3.13/3.11）
- 已安装并登录 Codex CLI
...
.\.venv-gpu\Scripts\python.exe backend\app.py
```
...
```powershell
.\.venv-gpu\Scripts\python.exe tools\regenerate_video_notes_backend.py --manifest output\my_videos.json --only BVxxxxxxxxxx
```
...
- 两台机器通过单独的 SMB 队列目录交换任务状态和产物，不把项目目录当队列，不共享 Codex 登录态，也不要求 GPU 主机常开。
- 人和任务计划程序只使用统一入口 `tools\start_worker.ps1 -Role cpu|gpu`；CPU/GPU 差异在 role 分支里处理。

典型命令：

```powershell
tools\distributed_enqueue.cmd --queue-root \\MINIPC\m2m_queue\_queue --manifest output\course.json
...
```powershell
.\.venv-cpu\Scripts\python.exe tools\rebuild_note_assets.py --refresh-screenshots
.\.venv-cpu\Scripts\python.exe tools\rebuild_note_assets.py "课程或演讲标题"
```
...
```powershell
.\.venv-cpu\Scripts\python.exe tools\rename_output_dirs.py --dry-run
.\.venv-cpu\Scripts\python.exe tools\rename_output_dirs.py
```

...
- 赛博洗稿：给定多个相关主题文章 URL 和深度提示词，自动提取、融合并通过 AI 深化为一篇综合性知识笔记。
...
将多篇相关主题文章 URL 转换为一篇高密度综合性知识笔记：

```powershell
# 准备文章清单 manifest（示例见 tools\article_manifest.sample.json）
# 运行洗稿
tools\article_washing.cmd --manifest output\my_articles.json --context-prompt "这些文章是关于 Unreal Engine GAS 技能系统的" --refinement-prompt "请深入解析 GAS 的核心概念、架构设计、实战技巧和常见陷阱"

# 指定输出目录
tools\article_washing.cmd --manifest output\my_articles.json --context-prompt "..." --refinement-prompt "..." --output-dir output\washed_notes

# 调大 token 预算（长文深度处理）
tools\article_washing.cmd --manifest output\my_articles.json --context-prompt "..." --refinement-prompt "..." --max-tokens 16384 --timeout 1200
```

也可以通过前端工作台直接操作：左侧菜单选择「赛博洗稿」，填入多个文章 URL、上下文提示和洗稿提示，点击开始即可生成。

...
| CPU 环境 | `.venv-cpu`，负责 Codex worker、前端看板后端和轻量脚本 |
...
| 队列产物目录 | `_queue\artifacts\`，跨机器交换 prepare/codex 产物，不手工编辑 |
...
D:\StudyReference\m2m_queue\
  AI-Media2Doc\                    项目源码；CPU 机和 GPU 机都应使用本机磁盘克隆
    backend\                       FastAPI 后端，媒体处理、ASR、Codex 调用和队列 API
...
`notes_raw.md` 保留 Codex 原始输出；`notes.md` 会把 `#image[整数秒]` 转为真实截图引用。质量文件记录字数、截图数量、章节数量、自然段数量、列表占比和是否通过重试。
